...
import (
	...
	"big-black-box/packages/utility/random"
	"math"
)

func Perlin(x, y, scale float32, seed float32) float32 {
	...
	var n00 = dot(intSeed, x0, y0, x, y)
	var n10 = dot(intSeed, x1, y0, x, y)
	var n01 = dot(intSeed, x0, y1, x, y)
	var n11 = dot(intSeed, x1, y1, x, y)
	return (lerp(lerp(n00, n10, sx), lerp(n01, n11, sx), sy) + 1) / 2
}
func OpenSimplex(x, y, scale float32, seed float32) float32 {
	var intSeed = floatToIntSeed(seed)
	...
	var getPerm = func(i, j int) int {
		var subSeed = random.CombineSeeds(intSeed, i)
		subSeed = random.CombineSeeds(subSeed, j)
...
func Worley(x, y, scale float32, seed float32) float32 {
	x *= scale
	y *= scale
	...
	for dy := int(-1); dy <= 1; dy++ {
		for dx := int(-1); dx <= 1; dx++ {
			...
			var cellSeed = random.CombineSeeds(intSeed, ix)
			cellSeed = random.CombineSeeds(cellSeed, iy)
...
func Voronoi(x, y, scale float32, seed float32) float32 {
	x *= scale
	y *= scale
	...
	var regionID = random.CombineSeeds(floatToIntSeed(seed), closestFeature[0])
	regionID = random.CombineSeeds(regionID, closestFeature[1])
	var regionVal = random.Range(0.0, 1.0, float32(regionID))
	return float32(regionVal)
}
func Value(x, y, scale float32, seed float32) float32 {
	x *= scale
	y *= scale
	...
	var seed00 = random.CombineSeeds(intSeed, xi)
	seed00 = random.CombineSeeds(seed00, yi)
	var seed10 = random.CombineSeeds(intSeed, xi+1)
	seed10 = random.CombineSeeds(seed10, yi)
	var seed01 = random.CombineSeeds(intSeed, xi)
	seed01 = random.CombineSeeds(seed01, yi+1)
	var seed11 = random.CombineSeeds(intSeed, xi+1)
	seed11 = random.CombineSeeds(seed11, yi+1)
...
func ValueCubic(x, y, scale float32, seed float32) float32 {
	x *= scale
	y *= scale
	...
	for gy := -1; gy <= 2; gy++ {
		for gx := -1; gx <= 2; gx++ {
			var cellSeed = random.CombineSeeds(intSeed, xi+int(gx))
			cellSeed = random.CombineSeeds(cellSeed, yi+int(gy))
...
func dot(intSeed, ix, iy int, x, y float32) float32 {
	var hash = random.CombineSeeds(intSeed, ix)
	hash = random.CombineSeeds(hash, iy)
